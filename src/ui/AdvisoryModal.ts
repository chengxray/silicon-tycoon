          ...
          <div class="flex justify-end gap-3 pt-3 border-t border-slate-700/60">
            ${
              diagnostic.category === 'CAPACITY' && onNavigateStore
                ? `<button id="btn-advisory-store" class="btn-sci-fi bg-cyan-600 hover:bg-cyan-500 font-bold">
                    🛒 前往商城增購設備分流
                  </button>`
                : ''
            }
            ${
              diagnostic.category === 'MAINTENANCE' && onNavigateHR
                ? `<button id="btn-advisory-hr" class="btn-sci-fi bg-amber-600 hover:bg-amber-500 font-bold">
                    🔧 前往人資指派維修保養
                  </button>`
                : ''
            }
